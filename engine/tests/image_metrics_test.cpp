...
TEST_CASE("analyze_exposure leaves input unchanged")
{
    ...
    static_cast<void>(keepers::analyze_exposure(grayscale));
...
TEST_CASE("percentile_contrast rejects empty input")
{
    ...
    REQUIRE_THROWS_AS(
        keepers::percentile_contrast(grayscale),
        std::invalid_argument
    );
}
...
TEST_CASE("percentile_contrast detects high-contrast input")
{
    ...
    REQUIRE(keepers::percentile_contrast(grayscale) > 0.0);
}
...
TEST_CASE("percentile_contrast is independent of input ordering")
{
    ...
    REQUIRE(
        keepers::percentile_contrast(ordered) ==
        Catch::Approx(keepers::percentile_contrast(shuffled))
    );
}
...
TEST_CASE("percentile_contrast leaves input unchanged")
{
    ...
    static_cast<void>(keepers::percentile_contrast(grayscale));
